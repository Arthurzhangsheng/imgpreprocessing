...
subprocess.call(["ffmpeg", '-i',frame_full_name, "-c:v", "libx264", "-vf", "fps=25,format=yuv420p",temp_out])

#声音合成
sound_dir = os.path.join(vid_dir,shortname+".aac")
final_vid_name = os.path.join(merge_dir,shortname+"out.mp4")
subprocess.call(["ffmpeg", "-i", temp_out, "-i", sound_dir, "-vcodec", "copy", "-acodec", "copy", final_vid_name])
