...
root_dir = r"D:\seagate2\DEEPFAKES\备选\other"
...
def getfile(root_dir):
    if os.path.isdir(root_dir):
        for _, _, files in os.walk(root_dir):
            for file in files:
                #if str(file).lower().endswith('.mp4') or str(file).lower().endswith('.avi') or str(file).lower().endswith('.mkv'):
                #获取文件名,不含后缀
                ...
                try:
                    ...
                    print("处理{}时出错".format(file))
                    fail_list.append(file)
            print("共处理了{}个视频".format(str(len(file_list))))
            print("失败了{}个视频".format(str(len(fail_list))))
    else:
